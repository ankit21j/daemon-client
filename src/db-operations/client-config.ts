...
export const updateDoc = (collection, data) => {
  ...
  collection.updateOne({ "authToken" : { "$exists" : true }} ,{ "$set" : jsonData })
    .then((result) => {
      return result  
    })
    .catch((error) => {
...
export const fetchEnabledLines = (db) => {
  return new Promise((resolve, reject) => {
    db.collection('clientConfig').find({}).project({'_id':0,'lines' : 1 }).toArray((err, doc) =>{
        if(err){
          logger.error(err)
          reject(err)
        }
...
export const fetchUidLimits = (db) => {
  return new Promise((resolve, reject) => {
    db.collection('clientConfig').find({}).project({'_id':0,'uidLimits' : 1 }).toArray((err, doc) =>{
        if(err){
          logger.error(err)
          reject(err)
        }
